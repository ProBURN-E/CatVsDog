...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, criterion, metric_ftns, optimizer, config, device,
        ...
        SummaryWriter().add_graph(self.model, next(iter(self.data_loader))[0].to(self.device)) # todo: more elegant way to get input shape
    ...
    def _valid_epoch(self, epoch):
        ...
        valid_bar = tqdm(self.valid_data_loader, desc='Validation')
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(valid_bar):
                data, target = data.to(self.device), target.to(self.device)
